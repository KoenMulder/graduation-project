      subroutine bicgstab2 (okprint,l, n, x, rhs, nonzero, tol, delta,
     &                      typestop, mxmv, work, ldw, info, typmv)
C 
C Simple BiCGstab(\ell) iterative method, \ell <= 2
C By M.A.Botchev, Jan.'98   OpenMP version by Bert Vreman 
...
C ----------------------------------------------------------
      implicit none
      integer l, n, mxmv, ldw, info, typmv
      real*8  x(n), rhs(n), tol, delta
...
C     -----------------------------------------
      integer   lmax
      parameter(lmax=2)
      real*8 rwork(lmax+1,3+2*(lmax+1))

      logical GoOn, rcmp, xpdt
      integer ii, i1, jj, kk, nmv
      real*8 alpha,beta,hatgamma,kappa0,kappal,
...
C      Initialize first residual
      if (nonzero) then
         call matvec (n, x, work(1,r) ,typmv)
...
!$omp parallel do private(ii)
      ...
      call sumprod(n,work(1,r),work(1,r),sum1)
      rnrm0 = sqrt( sum1 )
      rnrm = rnrm0
      if (typestop.eq.'max') call maxnorm(n,work(1,r),rnrmMax)
...
C     =====================
         rho0 = -omega*rho0
         do kk=1,l
            call sumprod(n,work(1,rr),work(1,r+kk-1),rho1)
...
!$omp parallel do private(ii)
            ...
            call matvec(n, work(1,u+kk-1), work(1,u+kk), typmv)
            nmv = nmv+1

            call sumprod(n,work(1,rr),work(1,u+kk),sigma)
...
!$omp parallel do private(ii)
            ...
            call matvec (n, work(1,r+kk-1), work(1,r+kk), typmv)
            nmv = nmv+1
...
C        Z = R'R
         do i1=1,l+1 
            do jj=i1-1,l
               call sumprod(n,work(1,r+jj),work(1,r+i1-1),sum1)
...
C     ================================
         ...
         if(typestop.eq.'rel')then
            ...
            call maxnorm(n,work(1,r),rnrmMax)
            GoOn = rnrmMax.ge.tol    .and. nmv.lt.mxmv
