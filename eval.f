...
!$omp parallel do 
     ...
     $   (x3_k(k)-zc)**2.le.radius**2) then
      ...
      do j=0,jm
      do i=0,im
          I_ave=I_ave+I_node(i,j,k)
      end do
      end do
      end do

      I_ave=I_ave/(im+1)/(jm+1)/(km+1)
